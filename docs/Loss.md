...
$$
Loss = -\sum_{i=1}\underbrace{y_i}_{\text{truth values}}\cdot\log{\underbrace{\hat{y_i}}_{\text{predicted values}}}
$$

### Foward pass: `forward(self, y_pred, y_true)`
- Input: `y_pred`, `y_true`
- Output: `loss`
```python
...
$$
\underbrace{\text{One-hot Vector} = \begin{pmatrix} 
0 & 1 & \cdots & a_{1,j} \\ 
1 & 0 & \cdots & a_{2,j} \\ 
\vdots & \vdots & \ddots & \vdots \\ 
a_{i,1} & a_{i,2} & \cdots & a_{i,j} 
\end{pmatrix}}_{\text{truth values}}
$$
...
\end{pmatrix}}_{\text{predicted values}}
$$
...
$$y\_pred = \begin{pmatrix} 0.7 & 0.2 & 0.1 \\ 0.1 & 0.8 & 0.1 \\ 0.3 & 0.6 & 0.1 \end{pmatrix}$$

Numpy fancy indexing `y_pred[range(len(y_pred)), y_true]` selects:

$$\begin{pmatrix} y\_pred[0,~ 0] \\ y\_pred[1,~ 1] \\ y\_pred[2,~ 1] \end{pmatrix} = \begin{pmatrix} 0.7 \\ 0.8 \\ 0.6 \end{pmatrix}$$
...
$$
\text{Binary Cross Entropy} = -\frac{1}{n}\sum_{i=1}^{n}y \cdot \log{\hat{y}} + (1 - y) \cdot \log{(1 - \hat{y})}
$$
...
## Backward Pass `backward(self, dvalues, y)`:
...
$$
\text{Mean Squared Error} = \frac{1}{n}\sum_{i=1}^{n}(\underbrace{y_i}_{\text{truth values}} - \underbrace{\hat{y_i}}_{\text{predicted values}})^2
$$
...
Directly translate to the python code above.
...
We let $L = \frac{1}{n}\sum_{i=1}^{n}(y_i - \hat{y_i})^2$ where we differentiate $L$ w.r.t $\hat{y}$.

$$
\frac{\partial L}{\partial \hat{y}_i} = \frac{\partial}{\partial \hat{y}_i}(\frac{1}{n}\sum_{i=1}^{n}(y_i - \hat{y_i})^2) 
$$

Deriving it is simply:

$$
\frac{\partial L}{\partial \hat{y}_i} = -2 \cdot \frac{1}{n}(y_i - \hat{y_i}) 
$$

The $\sum$ collapses because when differentiating w.r.t. a specific $\hat{y}_i$, 
all other terms in the sum vanish since they do not depend on $\hat{y}_i$.